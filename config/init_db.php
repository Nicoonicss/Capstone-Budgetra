...
// ── Column migrations (safe to run every request) ─────────────────────────
// Checks INFORMATION_SCHEMA before each ALTER so it never fails on re-runs.
$db_name = $pdo->query("SELECT DATABASE()")->fetchColumn();

$column_migrations = [
    ['users', 'country',         "ALTER TABLE users ADD COLUMN country VARCHAR(100) DEFAULT NULL"],
    ['users', 'currency_code',   "ALTER TABLE users ADD COLUMN currency_code VARCHAR(10) DEFAULT 'USD'"],
    ['users', 'currency_symbol', "ALTER TABLE users ADD COLUMN currency_symbol VARCHAR(10) DEFAULT '\$'"],
];

foreach ($column_migrations as [$table, $column, $alter_sql]) {
    $exists = $pdo->prepare(
        "SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS
         WHERE TABLE_SCHEMA = ? AND TABLE_NAME = ? AND COLUMN_NAME = ?"
    );
    $exists->execute([$db_name, $table, $column]);
